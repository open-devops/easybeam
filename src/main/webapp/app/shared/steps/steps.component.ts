...
export class TdStepsComponent implements OnDestroy, AfterContentInit {
  ...
  @Input('modelSteps') modelSteps: any[];
  ...
  areStepsActive(): boolean {
    return this.steps.filter((step: TdStepComponent) => {
      return step.active;
    }).length > 0;
  }
  ...
  private _deactivateAllBut(activeStep: TdStepComponent): void {
    this.steps.filter((step: TdStepComponent) => step !== activeStep)
  ...
  private _registerSteps(): void {
    this._subcriptions = [];
    this.steps.forEach((step: TdStepComponent) => {
